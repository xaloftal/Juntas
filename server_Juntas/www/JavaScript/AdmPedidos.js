    ...
    $.ajax({
        ...
        .then((response) => {
            ...
            response.forEach(pedido => {
                console.log(pedido);
                containerPedido.innerHTML += '<td>' + pedido.nome_u + '<p class="med-email">' + pedido.email_u + '</p></td><td class="right">' + pedido.data_pedido + '</td><td><button class="evaluate-btn">Avaliar</button></td>';
            })
